...
import { GoogleGenAI, GenerateContentParameters, HarmProbability, FinishReason } from '@google/genai';
import type { NodeType } from '../types';

// We no longer initialize a global 'ai' instance with a static key.
// Instead, we create instances on the fly or pass the key.
...
const buildParts = (inputs: Input[], instruction: string): any[] => {
    const parts: any[] = [];
...
const handleApiError = (response: any): string => {
    ...
    if (promptFeedback?.blockReason) {
        ...
        const blockedRating = promptFeedback.safetyRatings?.find((r: any) => r.probability !== HarmProbability.NEGLIGIBLE && r.probability !== HarmProbability.LOW && r.probability !== HarmProbability.HARM_PROBABILITY_UNSPECIFIED);
        if (blockedRating) {
...
export const runNode = async (
    ...
    if (!keyToUse || keyToUse === 'dummy_key_for_ui_dev') {
        throw new Error("API Key 缺失。请在设置中绑定您的 Google AI Studio Key。");
    }
